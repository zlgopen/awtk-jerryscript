...
jerry_value_t get_RESOURCE_TYPE_STYLE(
    const jerry_value_t func_obj_val, 
    const jerry_value_t this_p, 
    const jerry_value_t args_p[], 
    const jerry_length_t args_cnt
  ) {
  return jerry_create_number(RESOURCE_TYPE_STYLE);
}
...
ret_t resource_type_t_init(void) {
  ...
  jerryx_handler_register_global((const jerry_char_t*)"RESOURCE_TYPE_STYLE", get_RESOURCE_TYPE_STYLE);
...
jerry_value_t get_WIDGET_PROP_SUB_THEME(
  ...
  return jerry_create_str(WIDGET_PROP_SUB_THEME);
}
...
jerry_value_t wrap_widget_t_get_prop_style_name(
  ...
  return jerry_create_str(obj->style_name);
}
