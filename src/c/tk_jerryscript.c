...
jsvalue_t get_EVT_ERROR(const jerry_value_t func_obj_val, const jerry_value_t this_p,
                        const jerry_value_t argv[], const jerry_length_t argc) {
  void* ctx = NULL;
...
jsvalue_t wrap_assets_manager_set_theme(const jerry_value_t func_obj_val,
                                        const jerry_value_t this_p, const jerry_value_t argv[],
                                        const jerry_length_t argc) {
  void* ctx = NULL;
  jsvalue_t jret = JS_NULL;
...
jsvalue_t get_STYLE_ID_HIGHLIGHT_FONT_NAME(const jerry_value_t func_obj_val,
                                           const jerry_value_t this_p, const jerry_value_t argv[],
                                           const jerry_length_t argc) {
  void* ctx = NULL;
...
jsvalue_t get_STYLE_ID_HIGHLIGHT_FONT_SIZE(const jerry_value_t func_obj_val,
                                           const jerry_value_t this_p, const jerry_value_t argv[],
                                           const jerry_length_t argc) {
  void* ctx = NULL;
...
jsvalue_t get_STYLE_ID_HIGHLIGHT_TEXT_COLOR(const jerry_value_t func_obj_val,
                                            const jerry_value_t this_p, const jerry_value_t argv[],
                                            const jerry_length_t argc) {
  void* ctx = NULL;
...
jsvalue_t get_VGCANVAS_LINE_CAP_ROUND(const jerry_value_t func_obj_val, const jerry_value_t this_p,
                                      const jerry_value_t argv[], const jerry_length_t argc) {
  void* ctx = NULL;
  return jsvalue_create_string(ctx, VGCANVAS_LINE_CAP_ROUND);
}

jsvalue_t get_VGCANVAS_LINE_CAP_SQUARE(const jerry_value_t func_obj_val, const jerry_value_t this_p,
                                       const jerry_value_t argv[], const jerry_length_t argc) {
  void* ctx = NULL;
...
jsvalue_t get_VGCANVAS_LINE_JOIN_ROUND(const jerry_value_t func_obj_val, const jerry_value_t this_p,
                                       const jerry_value_t argv[], const jerry_length_t argc) {
  void* ctx = NULL;
  return jsvalue_create_string(ctx, VGCANVAS_LINE_JOIN_ROUND);
}

jsvalue_t get_VGCANVAS_LINE_JOIN_BEVEL(const jerry_value_t func_obj_val, const jerry_value_t this_p,
                                       const jerry_value_t argv[], const jerry_length_t argc) {
  void* ctx = NULL;
  return jsvalue_create_string(ctx, VGCANVAS_LINE_JOIN_BEVEL);
}

jsvalue_t get_VGCANVAS_LINE_JOIN_MITTER(const jerry_value_t func_obj_val,
                                        const jerry_value_t this_p, const jerry_value_t argv[],
                                        const jerry_length_t argc) {
  void* ctx = NULL;
...
jsvalue_t get_WIDGET_PROP_CARET_Y(const jerry_value_t func_obj_val, const jerry_value_t this_p,
                                  const jerry_value_t argv[], const jerry_length_t argc) {
  void* ctx = NULL;
...
jsvalue_t get_WIDGET_PROP_KEYBOARD(const jerry_value_t func_obj_val, const jerry_value_t this_p,
                                   const jerry_value_t argv[], const jerry_length_t argc) {
  void* ctx = NULL;
  return jsvalue_create_string(ctx, WIDGET_PROP_KEYBOARD);
}

jsvalue_t get_WIDGET_PROP_DEFAULT_FOCUSED_CHILD(const jerry_value_t func_obj_val,
                                                const jerry_value_t this_p,
                                                const jerry_value_t argv[],
                                                const jerry_length_t argc) {
  void* ctx = NULL;
...
jsvalue_t wrap_view_set_default_focused_child(const jerry_value_t func_obj_val,
                                              const jerry_value_t this_p,
                                              const jerry_value_t argv[],
                                              const jerry_length_t argc) {
  void* ctx = NULL;
...
jsvalue_t wrap_view_t_get_prop_default_focused_child(const jerry_value_t func_obj_val,
                                                     const jerry_value_t this_p,
                                                     const jerry_value_t argv[],
                                                     const jerry_length_t argc) {
  void* ctx = NULL;
...
jsvalue_t wrap_edit_set_keyboard(const jerry_value_t func_obj_val, const jerry_value_t this_p,
                                 const jerry_value_t argv[], const jerry_length_t argc) {
  void* ctx = NULL;
  jsvalue_t jret = JS_NULL;
  if (argc >= 2) {
...
jsvalue_t wrap_edit_t_get_prop_top_margin(const jerry_value_t func_obj_val,
                                          const jerry_value_t this_p, const jerry_value_t argv[],
                                          const jerry_length_t argc) {
  void* ctx = NULL;
  jsvalue_t jret = JS_NULL;
...
jsvalue_t wrap_edit_t_get_prop_bottom_margin(const jerry_value_t func_obj_val,
                                             const jerry_value_t this_p, const jerry_value_t argv[],
                                             const jerry_length_t argc) {
  void* ctx = NULL;
  jsvalue_t jret = JS_NULL;
...
jsvalue_t wrap_edit_t_get_prop_left_margin(const jerry_value_t func_obj_val,
                                           const jerry_value_t this_p, const jerry_value_t argv[],
                                           const jerry_length_t argc) {
  void* ctx = NULL;
  jsvalue_t jret = JS_NULL;
...
jsvalue_t wrap_edit_t_get_prop_right_margin(const jerry_value_t func_obj_val,
                                            const jerry_value_t this_p, const jerry_value_t argv[],
                                            const jerry_length_t argc) {
  void* ctx = NULL;
  jsvalue_t jret = JS_NULL;
...
jsvalue_t wrap_edit_t_get_prop_keyboard(const jerry_value_t func_obj_val,
                                        const jerry_value_t this_p, const jerry_value_t argv[],
                                        const jerry_length_t argc) {
  void* ctx = NULL;
  jsvalue_t jret = JS_NULL;
...
jsvalue_t wrap_done_event_cast(const jerry_value_t func_obj_val, const jerry_value_t this_p,
                               const jerry_value_t argv[], const jerry_length_t argc) {
  void* ctx = NULL;
  jsvalue_t jret = JS_NULL;
  if (argc >= 1) {
...
jsvalue_t wrap_done_event_t_get_prop_result(const jerry_value_t func_obj_val,
                                            const jerry_value_t this_p, const jerry_value_t argv[],
                                            const jerry_length_t argc) {
  void* ctx = NULL;
  jsvalue_t jret = JS_NULL;
...
jsvalue_t wrap_error_event_cast(const jerry_value_t func_obj_val, const jerry_value_t this_p,
                                const jerry_value_t argv[], const jerry_length_t argc) {
  void* ctx = NULL;
  jsvalue_t jret = JS_NULL;
  if (argc >= 1) {
...
jsvalue_t wrap_error_event_t_get_prop_code(const jerry_value_t func_obj_val,
                                           const jerry_value_t this_p, const jerry_value_t argv[],
                                           const jerry_length_t argc) {
  void* ctx = NULL;
  jsvalue_t jret = JS_NULL;
...
jsvalue_t wrap_error_event_t_get_prop_message(const jerry_value_t func_obj_val,
                                              const jerry_value_t this_p,
                                              const jerry_value_t argv[],
                                              const jerry_length_t argc) {
  void* ctx = NULL;
...
jsvalue_t wrap_progress_circle_set_line_cap(const jerry_value_t func_obj_val,
                                            const jerry_value_t this_p, const jerry_value_t argv[],
                                            const jerry_length_t argc) {
  void* ctx = NULL;
  jsvalue_t jret = JS_NULL;
...
jsvalue_t wrap_progress_circle_t_get_prop_line_cap(const jerry_value_t func_obj_val,
                                                   const jerry_value_t this_p,
                                                   const jerry_value_t argv[],
                                                   const jerry_length_t argc) {
  void* ctx = NULL;
...
jsvalue_t wrap_mledit_set_keyboard(const jerry_value_t func_obj_val, const jerry_value_t this_p,
                                   const jerry_value_t argv[], const jerry_length_t argc) {
  void* ctx = NULL;
  jsvalue_t jret = JS_NULL;
  if (argc >= 2) {
...
jsvalue_t wrap_mledit_t_get_prop_top_margin(const jerry_value_t func_obj_val,
                                            const jerry_value_t this_p, const jerry_value_t argv[],
                                            const jerry_length_t argc) {
  void* ctx = NULL;
  jsvalue_t jret = JS_NULL;
...
jsvalue_t wrap_mledit_t_get_prop_bottom_margin(const jerry_value_t func_obj_val,
                                               const jerry_value_t this_p,
                                               const jerry_value_t argv[],
                                               const jerry_length_t argc) {
  void* ctx = NULL;
...
jsvalue_t wrap_mledit_t_get_prop_left_margin(const jerry_value_t func_obj_val,
                                             const jerry_value_t this_p, const jerry_value_t argv[],
                                             const jerry_length_t argc) {
  void* ctx = NULL;
  jsvalue_t jret = JS_NULL;
...
jsvalue_t wrap_mledit_t_get_prop_right_margin(const jerry_value_t func_obj_val,
                                              const jerry_value_t this_p,
                                              const jerry_value_t argv[],
                                              const jerry_length_t argc) {
  void* ctx = NULL;
...
jsvalue_t wrap_mledit_t_get_prop_keyboard(const jerry_value_t func_obj_val,
                                          const jerry_value_t this_p, const jerry_value_t argv[],
                                          const jerry_length_t argc) {
  void* ctx = NULL;
  jsvalue_t jret = JS_NULL;
...
ret_t mledit_t_init(JSContext* ctx) {
  ...
  jerryx_handler_register_global((const jerry_char_t*)"mledit_t_get_prop_top_margin",
                                 wrap_mledit_t_get_prop_top_margin);
  jerryx_handler_register_global((const jerry_char_t*)"mledit_t_get_prop_bottom_margin",
                                 wrap_mledit_t_get_prop_bottom_margin);
  jerryx_handler_register_global((const jerry_char_t*)"mledit_t_get_prop_left_margin",
                                 wrap_mledit_t_get_prop_left_margin);
  jerryx_handler_register_global((const jerry_char_t*)"mledit_t_get_prop_right_margin",
                                 wrap_mledit_t_get_prop_right_margin);
...
jsvalue_t wrap_image_value_set_click_add_delta(const jerry_value_t func_obj_val,
                                               const jerry_value_t this_p,
                                               const jerry_value_t argv[],
                                               const jerry_length_t argc) {
  void* ctx = NULL;
...
jsvalue_t wrap_image_value_set_min(const jerry_value_t func_obj_val, const jerry_value_t this_p,
                                   const jerry_value_t argv[], const jerry_length_t argc) {
  void* ctx = NULL;
  jsvalue_t jret = JS_NULL;
  if (argc >= 2) {
...
jsvalue_t wrap_image_value_set_max(const jerry_value_t func_obj_val, const jerry_value_t this_p,
                                   const jerry_value_t argv[], const jerry_length_t argc) {
  void* ctx = NULL;
  jsvalue_t jret = JS_NULL;
  if (argc >= 2) {
...
jsvalue_t wrap_image_value_t_get_prop_click_add_delta(const jerry_value_t func_obj_val,
                                                      const jerry_value_t this_p,
                                                      const jerry_value_t argv[],
                                                      const jerry_length_t argc) {
  void* ctx = NULL;
...
jsvalue_t wrap_image_value_t_get_prop_min(const jerry_value_t func_obj_val,
                                          const jerry_value_t this_p, const jerry_value_t argv[],
                                          const jerry_length_t argc) {
  void* ctx = NULL;
  jsvalue_t jret = JS_NULL;
...
jsvalue_t wrap_image_value_t_get_prop_max(const jerry_value_t func_obj_val,
                                          const jerry_value_t this_p, const jerry_value_t argv[],
                                          const jerry_length_t argc) {
  void* ctx = NULL;
  jsvalue_t jret = JS_NULL;
